...
public class DoctorViewController : ControllerBase
{
    ...
    public DoctorViewController(
        ...
        ILogger<DoctorViewController> logger)
    {
        _context = context;
    ...
    public async Task<IActionResult> ProcessAiAssistant([FromBody] DoctorAiConversation dto)
    {
        try
        {
            ...
            var role = User.Claims.FirstOrDefault(c => c.Type == System.Security.Claims.ClaimTypes.Role || c.Type == "role")?.Value ?? "Doctor";
            var userName = User.Claims.FirstOrDefault(c => c.Type == System.Security.Claims.ClaimTypes.Name || c.Type == "unique_name" || c.Type == "username")?.Value ?? dto.DoctorName ?? "Attending Physician";

            var copilotResult = await _aiOrchestrationService.ExecuteDoctorCopilotQueryAsync(new AiCopilotQueryDto
            {
                ...
                TargetRole = "Doctor"
            }, callerRole: role, callerUser: userName);

            dto.Id = Guid.NewGuid();
            dto.PatientName = copilotResult.PatientName;
            dto.PatientIdCode = copilotResult.PatientIdCode;
            dto.AiResponse = copilotResult.ResponseText;
            dto.CreatedDate = DateTime.UtcNow;
        ...
        {
            _logger.LogError(ex, "Error processing Doctor AI assistant prompt.");
    ...
    public async Task<IActionResult> ProcessNurseAiAssistant([FromBody] DoctorAiConversation dto)
    {
        try
        {
            ...
            var role = User.Claims.FirstOrDefault(c => c.Type == System.Security.Claims.ClaimTypes.Role || c.Type == "role")?.Value ?? "Nurse";
            var userName = User.Claims.FirstOrDefault(c => c.Type == System.Security.Claims.ClaimTypes.Name || c.Type == "unique_name" || c.Type == "username")?.Value ?? dto.DoctorName ?? "Bedside Staff Nurse";

            var copilotResult = await _aiOrchestrationService.ExecuteNurseCopilotQueryAsync(new AiCopilotQueryDto
            {
                PatientName = dto.PatientName,
                PatientIdCode = dto.PatientIdCode,
                PromptQuery = dto.PromptQuery,
