...
public sealed class AuthHelper
{
    ...
    public async Task<string> LoginAsync(string username, string password, string role)
    {
        ...
        if (status != HttpStatusCode.OK)
            throw new InvalidOperationException($"Login failed for {role}/{username}: {status} {body.RootElement}");

        var data = ApiClient.Data(body);
        var token = ApiClient.String(data, "token");

        if (string.IsNullOrWhiteSpace(token))
            throw new InvalidOperationException($"Login response did not contain token: {body.RootElement}");
