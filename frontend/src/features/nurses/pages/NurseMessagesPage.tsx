...
export const NurseMessagesPage: React.FC = () => {
  ...
  // Shift & Call Modals / Toasts
  const [activeShift, setActiveShift] = useState('Day Shift (07:00 AM - 03:00 PM)');
        ...
        <div>
          <div className="flex items-center gap-2.5">
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">
              {isAdmin ? 'Messages & Operations Chat' : isDoctor ? 'Clinical Messages & Consults' : 'Messages & Shift Chat'}
            </h1>
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-indigo-50 text-indigo-700 border border-indigo-100">
              {isAdmin ? 'Admin Portal' : isDoctor ? 'Doctor Portal' : 'Nurse App'}
            </span>
          </div>
          <p className="text-xs font-semibold text-slate-500 mt-0.5">
            {isAdmin
              ? 'Coordinate facility operations, care teams, department heads, and shift notices.'
              : isDoctor
              ? 'Communicate directly with attending nurses, specialists, and care team members.'
              : 'Communicate securely with care team members, doctors, and pharmacy staff.'}
          </p>
        </div>

        {/* Header Right Controls */}
        <div className="flex flex-wrap items-center gap-3">
          {/* Shift Selector for Nurse */}
          {isNurse && (
            <div className="relative">
              <div
                onClick={() => setIsShiftDropdownOpen(!isShiftDropdownOpen)}
                className="flex items-center gap-2 px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 cursor-pointer hover:bg-slate-100 transition-colors"
              >
                <Sun className="h-4 w-4 text-amber-500 fill-amber-400" />
                <div className="flex flex-col text-[11px]">
                  <span className="font-extrabold text-slate-900">{activeShift.split('(')[0].trim()}</span>
                  <span className="text-[10px] text-slate-500 font-semibold">{activeShift.includes('(') ? activeShift.split('(')[1].replace(')', '') : 'Active Shift'}</span>
                </div>
                <ChevronDown className="h-3.5 w-3.5 text-slate-400 ml-1" />
              </div>

              {isShiftDropdownOpen && (
                <div className="absolute right-0 mt-1 w-64 bg-white border border-slate-200 rounded-xl shadow-xl p-2 z-50 text-xs font-semibold space-y-1">
                  {[
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-lg transition-colors cursor-pointer ${
                        activeShift === s ? 'bg-indigo-50 text-indigo-700 font-bold' : 'hover:bg-slate-50 text-slate-700'
                      }`}
                    >
                      {s}
                    </button>
          ...
          <div className="flex items-center gap-2.5 pl-2 border-l border-slate-200">
            ...
            <div className="text-left">
              ...
              <p className="text-[10px] font-semibold text-slate-400">
                {isAdmin ? 'System Administrator' : isDoctor ? (user?.specialty || 'Physician / Specialist') : 'Staff Nurse'}
              </p>
            </div>
          </div>
        </div>
