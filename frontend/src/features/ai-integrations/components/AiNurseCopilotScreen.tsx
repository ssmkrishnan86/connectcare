...
interface AiNurseCopilotScreenProps {
  patientId?: string;
  patientName?: string;
  onBack?: () => void;
}

export const AiNurseCopilotScreen: React.FC<AiNurseCopilotScreenProps> = ({
  patientId,
  patientName = 'Resident',
  onBack,
}) => {
  const toast = useToast();
  const [activeTab, setActiveTab] = useState<'Ask AI' | 'Bedside Guidance' | 'Patient Education' | 'SBAR Shift Summary'>('Ask AI');
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'm-init',
      sender: 'ai',
      text: `Hello Nurse. I am ready to assist with bedside care guidance, SBAR handover summaries, vital surveillance checks, and medication administration protocols for ${patientName}.`,
      modelVersion: 'gpt-4o',
  ...
  const quickPrompts = [
    `What are my top nursing shift priorities for ${patientName}?`,
    `Generate an SBAR shift handover note for next rotation.`,
    `Review patient education points for discharge preparation.`,
    `Check fall precaution bundle requirements for this resident.`
  ];
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      const res = await api.postAiNurseCopilot({
      ...
      console.error('Nurse Copilot error:', err);
      const errorMsg: Message = {
        id: `err-${Date.now()}`,
        sender: 'ai',
        text: `Unable to generate nursing response: ${err.message || 'AI service unavailable.'}. No fabricated clinical facts are presented.`,
  ...
  const handleCreateTaskFromAi = async (msgText: string) => {
    if (!patientId) {
      toast.info('Please select a resident to create a linked nursing task.');
      return;
    }
    ...
    try {
      await api.createTask({
        patientId: patientId,
        title: `[Nurse Copilot Task] Care protocol for ${patientName}`,
      ...
      toast.success('Nursing task dispatched to shift roster in ConnectCare.');
    } catch (err: any) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden flex flex-col font-sans">
      ...
      <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between bg-white">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-bold text-slate-900 flex items-center gap-2">
            ...
            <span>AI Nurse Copilot — {patientName}</span>
          </h1>
        </div>
      ...
      <div className="px-5 pt-3 border-b border-slate-100 flex items-center gap-1 overflow-x-auto bg-slate-50/30">
        {(['Ask AI', 'Bedside Guidance', 'Patient Education', 'SBAR Shift Summary'] as const).map((tab) => (
          <button
            key={tab}
      ...
      <div className="px-5 py-2.5 bg-slate-50/60 border-b border-slate-100 flex items-center gap-2 overflow-x-auto">
        <span className="text-[11px] font-bold text-slate-500 whitespace-nowrap">Bedside Protocols:</span>
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4 min-h-[380px]">
        ...
        <div className="space-y-4 overflow-y-auto max-h-[420px] pr-1">
          {messages.map((msg) => (
            ...
            >
              <div className="flex items-center gap-1.5 px-1 text-[10px] text-slate-400 font-semibold">
                <span>{msg.sender === 'user' ? 'Floor Nurse' : 'ConnectCare AI Nurse Copilot'}</span>
                ...
                {msg.sender === 'ai' && msg.safetyStatus !== 'Service Error' && (
                  <div className="pt-2 border-t border-slate-200/80 flex flex-wrap items-center justify-between gap-2 text-[11px]">
                    <span className="flex items-center gap-1 text-emerald-700 font-bold">
                      ...
                      <span>Nursing Policy Compliance Verified</span>
                    </span>
          ...
          {isTyping && (
            <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200 w-fit text-xs text-slate-500 font-medium">
              ...
              <span>Synthesizing nursing care guidance...</span>
            </div>
          )}
        </div>
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center gap-2">
          <input
            ...
            placeholder={`Ask Nurse AI Copilot regarding ${patientName}...`}
            disabled={isTyping}
      ...
      <AiEvidenceDrawer
        ...
        patientId={patientId}
        workflowName="Nurse AI Copilot"
      />
    </div>
  );
};

export default AiNurseCopilotScreen;
